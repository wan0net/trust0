<script lang="ts">
	...
	async function handleAddProof() {
		if (!identity || !handle.trim()) return;
		const cleanHandle = handle.trim().replace(/^@/, "");
		if (!/^[a-zA-Z0-9.-]+$/.test(cleanHandle)) {
			error = "Invalid Bluesky handle"; return;
		}
		const claimUri = `https://bsky.app/profile/${cleanHandle}`;
...
<div class="page-header">
	...
	<h1>Add Bluesky Proof</h1>
	<p>Link your Bluesky account to your identity</p>
</div>
...
{:else if success}
	<div class="card">
		...
		<div class="card-body">
			<p style="color: var(--green); font-weight: 500; margin-bottom: 16px;">Bluesky proof added successfully!</p>
...
{:else}
	...
	<div class="grid">
		<div class="card">
			...
			<div class="card-body">
				<p>Add this text to your Bluesky profile description:</p>
				<div class="proof-box">
					<code>aspe:login2.link42.app:{identity.fingerprint}</code>
					<button class="btn-sm secondary" onclick={copyProofText}>Copy</button>
				</div>
				<a href="https://bsky.app/settings" target="_blank" rel="noopener noreferrer" role="button" class="outline btn-sm" style="margin-top: 16px;">
		...
		<div class="card">
			<div class="card-header">Step 2: Enter Your Handle</div>
			<div class="card-body">
				<form onsubmit={(e) => { e.preventDefault(); handleAddProof(); }}>
					...
					<button type="submit" aria-busy={adding} disabled={adding || !handle.trim()}>Add Proof</button>
				</form>
			</div>
		</div>
	</div>
