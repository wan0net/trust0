<script lang="ts">
	...
	async function handleAddProof() {
		if (!identity || !gistUrl.trim()) return;
		if (!gistUrl.startsWith("https://gist.github.com/")) {
			error = "URL must be a GitHub Gist (https://gist.github.com/...)";
			return;
		}
		adding = true;
		error = null;
		try {
			...
			if (identity) {
				try {
					...
					if (chain) {
						await appendAfterAction(identity, chain.identityId, "proof_add", { claim_uri: gistUrl.trim() });
					}
				} catch (err) {
...
<div class="page-header">
	...
	<h1>Add GitHub Proof</h1>
	<p>Link your GitHub account to your identity</p>
</div>
	...
	<div class="card">
		...
		<div class="card-body">
			<p style="color: var(--green); font-weight: 500; margin-bottom: 16px;">
				GitHub proof added successfully!
			</p>
...
{:else}
	...
	<div class="grid">
		<div class="card">
			<div class="card-header">Step 1: Post Your Proof</div>
			<div class="card-body">
				<p>Create a public GitHub Gist containing this text:</p>
				<div class="proof-box">
					<code>aspe:login2.link42.app:{identity.fingerprint}</code>
					<button class="btn-sm secondary" onclick={copyProofText}>Copy</button>
				</div>
				<a href="https://gist.github.com" target="_blank" rel="noopener noreferrer" role="button" class="outline btn-sm" style="margin-top: 16px;">
					Open GitHub Gist ↗
				</a>
			</div>
		</div>
		...
		<div class="card">
			<div class="card-header">Step 2: Paste Gist URL</div>
			<div class="card-body">
				<form onsubmit={(e) => { e.preventDefault(); handleAddProof(); }}>
					<label>
						...
						<input 
							type="url" 
							bind:value={gistUrl} 
							placeholder="https://gist.github.com/username/abcdef..." 
							required 
						/>
					</label>
					<button type="submit" aria-busy={adding} disabled={adding || !gistUrl.trim()}>
						Add Proof
					</button>
				</form>
			</div>
