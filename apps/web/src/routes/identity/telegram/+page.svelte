<script lang="ts">
	...
	async function checkStatus() {
		...
		statusMessage = null;
		try {
			const res = await fetch(`${API_BASE}/api/identity/attestation-status/${identity.fingerprint}/telegram`);
			if (!res.ok) {
				statusMessage = "Not yet verified. Send the command to the bot and try again.";
				checking = false;
				return;
			}
			...
			if (data.attested && data.platformUserId) {
				...
				const claimUri = `telegram:${data.platformUserId}`;
				...
				try {
					...
					if (chain) {
						await appendAfterAction(identity, chain.identityId, "proof_add", { claim_uri: `telegram:${data.platformUserId}` });
					}
				} catch (err) {
...
<div class="page-header">
	...
	<h1>Add Telegram Proof</h1>
	<p>Link your Telegram account to your identity</p>
</div>
	...
	<div class="card">
		...
		<div class="card-body">
			<p style="color: var(--green); font-weight: 500; margin-bottom: 16px;">
				Telegram proof added successfully!
			</p>
...
{:else}
	...
	<div class="grid">
		<div class="card">
			<div class="card-header">Step 1: Message the Bot</div>
			<div class="card-body">
				<p>Send the following command to the Link42 Telegram bot:</p>
				<div class="proof-box">
				...
				<p style="color: var(--text-dim); font-size: 13px; margin-top: 12px;">
					The bot will verify your Telegram account and attest the link to your identity.
				</p>
			</div>
		</div>
