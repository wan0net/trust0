<script lang="ts">
	...
	async function handleAddProof() {
		...
		if (!/^[a-zA-Z0-9_-]+$/.test(name)) {
			error = "Username must be alphanumeric"; return;
		}
		const claimUri = `https://sr.ht/~${name}`;
...
<div class="page-header">
	...
	<h1>Add Sourcehut Proof</h1>
	<p>Link your Sourcehut account to your identity</p>
</div>
...
{:else if success}
	<div class="card">
		...
		<div class="card-body">
			<p style="color: var(--green); font-weight: 500; margin-bottom: 16px;">Sourcehut proof added successfully!</p>
...
{:else}
	...
	<div class="grid">
		<div class="card">
			...
			<div class="card-body">
				<p>Add this text to your Sourcehut profile bio:</p>
				<div class="proof-box">
					<code>aspe:login2.link42.app:{identity.fingerprint}</code>
					<button class="btn-sm secondary" onclick={copyProofText}>Copy</button>
				</div>
				<a href="https://meta.sr.ht/profile" target="_blank" rel="noopener noreferrer" role="button" class="outline btn-sm" style="margin-top: 16px;">
					Open Sourcehut Profile ↗
				</a>
			</div>
		</div>
