<script lang="ts">
	...
	async function handleAddProof() {
		if (!identity || !orcid.trim()) return;
		const id = orcid.trim();
		if (!/^\d{4}-\d{4}-\d{4}-\d{3}[\dX]$/.test(id)) {
			error = "ORCID must be in the format 0000-0002-1825-0097 (4 groups of 4 digits, last character may be X)";
			return;
		}
		const claimUri = `https://orcid.org/${id}`;
		adding = true;
		error = null;
		try {
			await updateProfile(
...
<div class="page-header">
	...
	<h1>Add ORCID Proof</h1>
	<p>Link your ORCID researcher profile to your identity</p>
</div>
	...
	<div class="card">
		...
		<div class="card-body">
			<p style="color: var(--green); font-weight: 500; margin-bottom: 16px;">
				ORCID proof added successfully!
			</p>
...
{:else}
	...
	<div class="grid">
		<div class="card">
			<div class="card-header">Step 1: Add Proof to Your ORCID Bio</div>
			<div class="card-body">
				...
				<a href="https://orcid.org/my-orcid" target="_blank" rel="noopener noreferrer" role="button" class="outline btn-sm" style="margin-top: 16px;">
					Open ORCID Profile Settings ↗
				</a>
				<p style="margin-top: 12px; font-size: 0.9em; color: var(--text-muted);">
					Go to your ORCID profile, click "Edit" on the biography section, and paste the proof text.
				</p>
			</div>
		</div>

		<div class="card">
			<div class="card-header">Step 2: Enter Your ORCID</div>
			<div class="card-body">
				<form onsubmit={(e) => { e.preventDefault(); handleAddProof(); }}>
					...
					<p style="font-size: 0.85em; color: var(--text-muted); margin-bottom: 12px;">
						Your claim will link to <code>https://orcid.org/{orcid.trim() || "..."}</code>
					</p>
					<button type="submit" aria-busy={adding} disabled={adding || !orcid.trim()}>
						Add Proof
					</button>
				</form>
			</div>
